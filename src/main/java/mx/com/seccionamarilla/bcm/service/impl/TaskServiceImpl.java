...
public class TaskServiceImpl implements ITaskService {
	...
	public void updateFlowTask(KafkaTaskRequest kTaskRequest, ProcessedMessage pm, String taskStatus) {
		...
		try {
			...
			int rows = taskDao.updateAssignUserAndTaskStatus(kTaskRequest.getUserId(), data.getTaskStatus(), kTaskRequest.getUserId(),
					Long.valueOf(kTaskRequest.getFlowInstanceId()));
			String strVarProcesado = "PROCESADO";
			if (rows == 0) {
				strVarProcesado = "NO_PROCESADO";
			}
			// No se actualizó nada → revisar criterios
			processMessagesService.updateStatusOut(pm.getProcessedMessagesId(), strVarProcesado, null, VAR_FLOW_INSTANCE_ID,
					kTaskRequest.getFlowInstanceId(), kTaskRequest.getProductCode(), null);
		} catch (DataAccessException dae) {
			processMessagesService.updateStatusOut(pm.getProcessedMessagesId(), "ERROR", dae.getMessage(), VAR_FLOW_INSTANCE_ID,
					kTaskRequest.getFlowInstanceId(), kTaskRequest.getProductCode(), dae.getLocalizedMessage());
			...
			processMessagesService.updateStatusOut(pm.getProcessedMessagesId(), "ERROR", e.getMessage(), VAR_FLOW_INSTANCE_ID,
					kTaskRequest.getFlowInstanceId(), kTaskRequest.getProductCode(), e.getLocalizedMessage());
